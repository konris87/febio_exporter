...
class Boundary:
    ...
    def add_zero_displacement(self, name, dofs, node_set_name, root=None):

        assert (dof in ['x_dof', 'y_dof', 'z_dof'] for dof in dofs)
        if root is None:
            self.root = self.parent.boundaries
        else:
            self.root = root

        zero_el = ET.SubElement(
            self.root, 'bc',
            attrib={
                'name': name,
                'node_set': node_set_name,
        ...
        for dof in dofs:
            dof_el = ET.SubElement(zero_el, dof)
            dof_el.text = str(1)
