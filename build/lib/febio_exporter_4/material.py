...
class Material:
	...
	def add_elastic_matrix(self, parameters, prestrain, root=None):
		"""
		...
		if root is None:
			...
			parent = root
		if prestrain:
			self.elastic_elem = ET.SubElement(parent, "elastic", attrib={
				"type": parameters["type"]
			})
			parent = self.elastic_elem
	...
	def add_fiber(self, parameters, f_type, root=None):
		"""
		...
		"""
		assert f_type in ["map", "vector", "local"], "Wrong Fiber Type"
		...
		# elif mixture:
		# 	parent = self.material.find("solid")[0]
		if root:
			parent = root
		else:
			parent = self.material
		...
		if isinstance(parameters[f_type], str):
			fiber_elem.text = str(parameters[f_type])
		else:
			fiber_elem.text = to_xml_field(parameters[f_type])

	def add_active_contraction(self, parameters, prestrain=0):
		"""
	...
	def add_solid_mixture(self, name, solid_type, mix_parameters, solid_tags):
		"""
		...
		solid_type :
		mix_parameters :
		solid_tags : dictionary containing tags for the added solids,
			e.g. {"type": "trans is Mooney-Rivlin"}

		Returns
		-------
		ET.Elements of the solid entries
		"""

		self.material = ET.SubElement(
			self.parent.materials, 'material',
			attrib={
				'id': str(self.parent.material_id + 1),
				'name': name,
				'type': solid_type})

		# add every value that is not a dictionary, these are the common
		# mixture parameters
		for key, value in mix_parameters.items():
			if key == "type":
				pass
			elif not isinstance(value, dict):
				elem = ET.SubElement(self.material, key)
		...
		# add solid parameters
		solids = []
		for item in solid_tags:
			solid = ET.SubElement(
				self.material, 'solid', attrib={"type": item['type']})
			solids.append(solid)

		self.parent.material_id += 1

		return self.parent.material_id, solids

	def add_fiber_stress(self, parameters, parent):
		"""
		...
		for key, value in parameters.items():
			if key == 'type':
				...
				act_elem = ET.SubElement(
					parent, key, attrib={
						"lc": str(self.parent.loadcurve_id + 1)})
				self.parent.loadcurve_id += 1
				act_elem.text = str(value)
			elif key in ["stl", 'stv']:
	...
	def add_unc_active_contraction(self, parameters, root=None):
		"""
		...
		for key, value in parameters.items():
			if key == 'type':
				...
				fib_elem = ET.SubElement(
					root, key, attrib={
						"type": value["type"]})
				fib_elem.text = value["value"]
			else:
	...
	def add_isotropic_elastic(self, name, d, E, v):
		...
		density = ET.SubElement(mat, 'density')
		density.text = str(d)

		youngMod = ET.SubElement(mat, 'E')
		youngMod.text = str(E)

		poisson = ET.SubElement(mat, 'v')
		poisson.text = str(v)
	...
	def add_porous_neo_hookean(self, name, d, E, phi):
		
		mat = ET.SubElement(
		self.parent.materials, 'material',
		attrib={'id': str(self.parent.material_id + 1),
				'name': name,
				'type': "porous neo-Hookean"})
		...
		porosity = ET.SubElement(mat, 'phi0')
		porosity.text = str(phi)

		self.parent.material_id += 1
		return self.parent.material_id
	
	def add_neo_hookean(self, name, d, E, v):
